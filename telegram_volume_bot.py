...
Features:
- /screen → P1 (10 rows), P2 (10 rows), P3 (10 rows; P3 always includes pinned: BTC,ETH,XRP,SOL,DOGE,ADA,PEPE,LINK)
...
def build_priorities(best_spot: Dict[str,MarketVol], best_fut: Dict[str,MarketVol]):
    ...
    # P1: Fut≥5M & Spot≥500k (exclude pinned)
    ...
    p1 = [r for r in p1_full if r[0] not in PINNED_SET][:TOP_N_P1]
    used.update(r[0] for r in p1)

    # P2: Fut≥2M (exclude pinned + used)
    ...
    p2 = [r for r in p2_full if r[0] not in PINNED_SET][:TOP_N_P2]
    used.update(r[0] for r in p2)

    # P3: pinned + Spot≥3M others
    p3_dict = {}
    for base in PINNED_P3:
        s = best_spot.get(base); f = best_fut.get(base)
    ...
    pinned_rows = [r for r in all_rows if r[0] in PINNED_SET]; pinned_rows.sort(key=lambda r: r[2], reverse=True)
    other_rows  = [r for r in all_rows if r[0] not in PINNED_SET]; other_rows.sort(key=lambda r: r[2], reverse=True)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 Commands:\n"
        "• /screen → P1(10), P2(10), P3(10) with SYM | F | S | % | %4H\n"
...
# ... (excel_cmd, diag, coin_query, main stay the same as before) ...
