...
def make_setup(
    ...
    try:
        ...
        override_mult = 0.35 if (not strict_15m) else 0.55
        soft_ok = (fut_vol >= 5_000_000.0) and (abs(ch24) >= 10.0) and (abs(ch1) >= float(trig_min) * float(override_mult))
        if abs(ch1) < trig_min and soft_ok:
            _note_status("ch1_soft_override", base, mv)
        if abs(ch1) < trig_min and (not soft_ok):
        ...
        # Never let a single symbol crash setup generation — fallback to a conservative ATR-based setup.
        try:
            _rej('exception_in_make_setup', base, mv, str(e)[:160])
        except Exception:
            pass
        entry_f = float(getattr(mv, 'last', 0.0) or 0.0)
        if entry_f <= 0:
            return None
        ch24_f = float(getattr(mv, 'percentage', 0.0) or 0.0)
        side_f = 'BUY' if ch24_f >= 0 else 'SELL'
        atr_1h_f = 0.0
        try:
            c1_f = fetch_ohlcv(getattr(mv, 'symbol', ''), '1h', limit=max(ATR_PERIOD + 10, 80))
            atr_1h_f = compute_atr_from_ohlcv(c1_f, ATR_PERIOD) if c1_f else 0.0
        except Exception:
            atr_1h_f = 0.0
        if atr_1h_f <= 0:
            atr_1h_f = max(entry_f * 0.01, 0.0000001)  # 1% default
        sl_dist = 1.4 * atr_1h_f
        tp_dist = 2.2 * atr_1h_f
        if side_f == 'BUY':
            sl_f = max(entry_f - sl_dist, entry_f * 0.001)
            tp3_f = entry_f + tp_dist
            tp1_f = entry_f + tp_dist * 0.6
            tp2_f = entry_f + tp_dist * 0.85
        else:
            sl_f = entry_f + sl_dist
            tp3_f = max(entry_f - tp_dist, entry_f * 0.001)
            tp1_f = max(entry_f - tp_dist * 0.6, entry_f * 0.001)
            tp2_f = max(entry_f - tp_dist * 0.85, entry_f * 0.001)
        fut_vol_f = 0.0
        try:
            fut_vol_f = float(usd_notional(mv) or 0.0)
        except Exception:
            fut_vol_f = 0.0
        return Setup(
