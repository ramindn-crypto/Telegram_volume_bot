...
# Setup logic (Trigger 1H, Confirm 15m)
TRIGGER_1H_ABS_MIN = 2.0
CONFIRM_15M_ABS_MIN = 0.6
ALIGN_4H_MIN = 0.0  # long wants 4H>=0, short wants 4H<=0

# Risk defaults
DEFAULT_EQUITY = 1000.0
DEFAULT_RISK_PCT = 1.0
DEFAULT_MAX_TRADES_DAY = 3
DEFAULT_DAILY_RISK_CAP = 50.0
DEFAULT_OPEN_RISK_CAP = 75.0

# SL/TP sizing
SL_PCT = 3.0
TP_PCT = 6.0

# Multi-TP only for Conf>=75
...
# Quotes we accept
STABLES = {"USDT", "USDC", "USD", "FDUSD", "TUSD", "DAI", "PYUSD"}
...
def regime_label(ch24: float, ch4: float) -> str:
    if ch24 >= 3 and ch4 >= 2: return "LONG 🟢"
    if ch24 <= -3 and ch4 <= -2: return "SHORT 🔴"
    return "NEUTRAL 🟡"
...
def compute_confidence(side: str, ch24: float, ch4: float, ch1: float, ch15: float, fut_vol_usd: float) -> int:
    ...
    if fut_vol_usd >= 15_000_000: score += 8
    elif fut_vol_usd >= 6_000_000: score += 6
    elif fut_vol_usd >= 2_000_000: score += 4
    elif fut_vol_usd >= 1_000_000: score += 2
...
def format_setup_email_block(s: Setup) -> str:
    snapshot = (
        ...
        f"1H {pct_with_emoji(s.ch1)} | 15m {pct_with_emoji(s.ch15)} | {s.regime}"
    )
    lines = []
    ...
    if s.conf >= MULTI_TP_MIN_CONF and s.tp1 and s.tp2:
        runner_hint = "trail EMA20 (1H) or last 15m swing-low" if s.side == "BUY" else "trail EMA20 (1H) or last 15m swing-high"
        lines.append(
...
- /tradesetup <SYMBOL> — Uses PulseFutures signal if available (no need to run /screen)
- /risk <SYMBOL> — same as /tradesetup
- /open — Show open positions
- /closepnl <SYMBOL> <pnlUSD> — Close oldest open position and update equity (e.g. /closepnl BTC +23.5)
...
async def diag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        f"*Diag*\n"
        f"- Exchange: `{EXCHANGE_ID}` futures\n"
...
def parse_symbol(text: str) -> str:
    return re.sub(r"[^A-Za-z0-9]", "", text).upper().lstrip("$")
...
async def tradesetup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not sig:
        ...
        mv = best_fut.get(sym)
        if not mv:
            await update.message.reply_text(f"{warning}Could not find {sym} on futures.")
            return
    ...
    if int(user["day_trade_count"]) >= int(user["max_trades_day"]):
        await update.message.reply_text("❌ Max trades per day reached.")
        return
    ...
    if qty <= 0 or notional <= 0:
        await update.message.reply_text("Could not compute position size.")
        return
...
async def alert_job(context: ContextTypes.DEFAULT_TYPE):
    try:
        if not EMAIL_ENABLED_DEFAULT or not email_config_ok():
            return
        if not window_ok():
            return
        ...
        cur_syms = {s.symbol for s in setups if s.symbol}
